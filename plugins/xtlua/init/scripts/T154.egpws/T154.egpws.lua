...
local find_datarefs = {
    { "simDR_qfeqnh", "tu154/custom/switchers/ovhd/egpws_mode" },
    { "simDR_vbe1", "tu154/custom/switchers/ovhd/vbe_1_on" },
    { "simDR_vbe2", "tu154/custom/switchers/ovhd/vbe_2_on" },
    { "simDR_var1", "tu154/custom/switchers/ovhd/var_left" },
    { "simDR_var2", "tu154/custom/switchers/ovhd/var_right" },
    { "simDR_svs", "tu154/custom/switchers/ovhd/svs_on" },
    { "simDR_on_ground", "sim/flightmodel/failures/onground_all" },
    { "simDR_diss", "tu154/custom/nvu/diss_mode" },
    { "simDR_gps_power", "sim/cockpit2/radios/actuators/gps_power" },
    { "simDR_taws1", "tu154/custom/taws/taws_message" },
    { "simDR_taws2", "tu154/custom/sounds/taws_rus_phrase" },
    { "simDR_taws3", "tu154/custom/sounds/taws_eng_phrase" },
    { "simDR_taws_mode", "tu154/custom/taws/mode_set" },
    { "simDR_alarm", "tu154/custom/switchers/ovhd/egpws_alarm_1" },
...
local deferred_datarefs = {
    { "dis_sound", "tu154/custom/egpws/dis_sound", "number" },
    { "dis_gear", "tu154/custom/egpws/dis_gear", "number" },
    { "dis_flaps", "tu154/custom/egpws/dis_flaps", "number" },
    { "dis_rppz", "tu154/custom/egpws/dis_rppz", "number" },
    { "dis_gs", "tu154/custom/egpws/dis_gs", "number" },
    { "srpbz", "tu154/custom/kontur/srpbz", "number" },
    { "srpbz_lit", "tu154/custom/kontur/srpbz_lit", "number" },
    { "srpbz_lit_set", "tu154/custom/kontur/srpbz_lit_set", "number" },
    { "srpbz_rppz_lit", "tu154/custom/kontur/srpbz_rppz_lit", "number" },
    { "srpbz_sppz_lit", "tu154/custom/kontur/srpbz_sppz_lit", "number" },
    { "srpbz_test_lit", "tu154/custom/kontur/srpbz_test_lit", "number" },
}

-- Publish into this xTlua script's proxied environment, not its parent _G.
-- All DataRef variables above remain globals with the same names and paths.
local script_env = getfenv(1)
local function bind_datarefs(definitions)
    for _, def in ipairs(definitions) do
        script_env[def[1]] = find_dataref(def[2])
    end
end
local function create_datarefs(definitions)
    for _, def in ipairs(definitions) do
        script_env[def[1]] = deferred_dataref(def[2], def[3], def[4])
    end
end
...
local function suppress_warning_audio()
    -- IDs 11..20 are warnings in both voice sets, not height callouts.
    if simDR_taws2 >= 11 and simDR_taws2 <= 20 then simDR_taws2 = 0 end
    if simDR_taws3 >= 11 and simDR_taws3 <= 20 then simDR_taws3 = 0 end
end
...
local function apply_alert_confirmation(dt)
    ...
    if simDR_alarm == 0 then
        suppress_warning_audio()
        reset_alert_filter()
        return
    end
...
function egpws()
    ...
    -- Read the overhead warning switch; never override the pilot's selection.
    -- SASL also gates the source and playing samples, so suppression is not
    -- dependent on the asynchronous order of these two plugins.
    if simDR_taws_mode < 5 then
        if dis_sound > 0 then
            suppress_message_audio()
        end
        ...
        if simDR_alarm == 0 then
            if simDR_taws1 ~= 2 and simDR_taws1 ~= 7 then simDR_taws1 = 0 end
            suppress_warning_audio()
        end
    end
