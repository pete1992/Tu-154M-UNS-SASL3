...
simDR_toelev_ratio = find_dataref("tu154/custom/controls/control_force_pos")
...
stab_work =  deferred_dataref("tu154/custom/lights/stab_work_new", "number")
slats_extended =  deferred_dataref("tu154/custom/lights/slats_extended_new", "number")
lit_torud_blinks =  deferred_dataref("tu154/custom/test", "number")
...
function blinks_lights()
  ...
  if slats_delta > 0.04 and bus27 > 0 then
    if ping_pong_slat < 10 and ping_pong_slat_round < 1 then
       ping_pong_slat = ping_pong_slat + 60 * SIM_PERIOD
    else
  ...
  elseif slats_delta < -0.04 and bus27 > 0 then
    ...
    if simDR_torud_ratio > 0.01 and simDR_torud_ratio < 0.99 and bus27 > 0 then
        if ping_pong_torud < 10 and ping_pong_torud_round < 1 then
           ping_pong_torud = ping_pong_torud + 60 * SIM_PERIOD
        else
    ...
    if simDR_toelev_ratio > 0.01 and simDR_toelev_ratio < 0.99 and bus27 > 0 then
        if ping_pong_toelev < 10 and ping_pong_toelev_round < 1 then
           ping_pong_toelev = ping_pong_toelev + 60 * SIM_PERIOD
        else
