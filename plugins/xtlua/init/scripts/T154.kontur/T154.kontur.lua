...
simDR_nav_select                    = find_dataref("tu154/custom/switchers/nav_select")
...
function kontur_btn3_button_l_CMDhandler(phase, duration)
     if phase == 0 then
        ...
        if kontur_wx_l > 1 and info_page_l < 1 then
...
if kontur_nav_menu_l > 2 and kontur_nav_l < 1 then
    ...
    elseif weather_mode > 0 and weather_sys > 0 and simDR_36v > 0 and kontur_wx_test_l == -1 and kontur_wx_r > 1 then
...
if radioalt_loc < 780 then  
  ...
  vvi_rv = 0
    ...
    if kontur_nav_r > 0 and uns2_on > 0 then
    simDR_fms_line = 0
    end 
