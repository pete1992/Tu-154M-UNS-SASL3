...
function draw()
	...
	sasl.gl.drawRectangle(155, 0, 10, size[2], 1, 1, 1, 1) -- vertical line
	...
	for i = math.floor((spd - 150) / 50) * 50, math.floor((spd + 150) / 50) * 50, 50 do
		if i % 100 == 0 then  -- big lines
			...
			if line_pos > 0 and line_pos < size[2] - 5 then
				sasl.gl.drawRectangle(135, line_pos, 20, 5, 1, 1, 1, 1) -- +100 line
			end
			...
			if text_pos > 0 and text_pos < size[2] - 30 then
				...
				if i < 1000 then
					sasl.gl.drawBitmapText(text_font, 50, text_pos, text, TEXT_ALIGN_LEFT, {1, 1, 1, 1})
				else
					sasl.gl.drawBitmapText(text_font, 25, text_pos, text, TEXT_ALIGN_LEFT, {1, 1, 1, 1})
				end
			...
			local line_pos = 168 + (i - spd) * 1.2
			if line_pos > 0 and line_pos < size[2] - 5 then
				sasl.gl.drawRectangle(125, line_pos, 30, 5, 1, 1, 1, 1) -- +50 line
			end
		end
