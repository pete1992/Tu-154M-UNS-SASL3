...
function draw()
	...
	sasl.gl.drawRectangle(18, 0, 10, size[2], 1, 1, 1, 1) -- vertical line
	...
	for i = math.floor((sec - 15) / 5) * 5, math.floor((sec + 15) / 5) * 5, 5 do
		...
		if i % 10 == 0 then  -- big lines
			...
			if line_pos > 0 and line_pos < size[2] - 5 then
				sasl.gl.drawRectangle(27, line_pos, 30, 5, 1, 1, 1, 1) -- +100 line
			end
			...
			if text_pos > 0 and text_pos < size[2] - 30 then
				...
				sasl.gl.drawBitmapText(text_font, 60, text_pos, text, TEXT_ALIGN_LEFT, {1, 1, 1, 1})
			end--]]
		else -- small lines
			local line_pos = 168 + (i - sec) * 11
			if line_pos > 0 and line_pos < size[2] - 5 then
				sasl.gl.drawRectangle(27, line_pos, 20, 5, 1, 1, 1, 1) -- +100 line
			end
		end
