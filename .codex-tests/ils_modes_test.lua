...
test("landing off clears LD and does not restore it without a new selection", function(r)
    r:capture(); eq(r:get("absu_pnp_mode_1"), 4, "captain LD selected")
    r:set("absu_landing_on", 0); r:step()
    eq(r:get("absu_pnp_mode_1"), 0, "captain LD cleared")
    r:step(100); eq(r:get("absu_pnp_mode_1"), 0, "LD stays off")
    r:set("absu_pnp_mode_1", 4); r:step()
    eq(r:get("absu_pnp_mode_1"), 0, "stale LD cannot persist with landing off")
    r:set("absu_landing_on", 1); r:step()
    eq(r:get("absu_pnp_mode_1"), 4, "new landing selection restores LD")
end)
for _, selection in ipairs({{"absu_nvu", 1}, {"absu_az1", 2}, {"absu_az2", 3}}) do
    test("landing off preserves explicit " .. selection[1] .. " selection", function(r)
        r:set("absu_nav_on", 1); r:set("absu_landing_on", 0)
        r:press(selection[1]); r:step(20)
        eq(r:get("absu_pnp_mode_1"), selection[2], "NAV/VOR has priority over stale LD")
    end)
end
test("copilot selector and manual ILS directors remain independent", function(r)
    r:capture(); r:set("absu_speed_mode", 4); r:step()
    r.handlers["sim/autopilot/fdir_toggle"](0); r:step()
    r:modes(1, 1, 6, 5, "manual flight directors retained")
    eq(r:get("absu_pnp_mode_1"), 4, "AP disconnect alone does not cancel LD")
    r:set("absu_landing_on", 0); r:step()
    eq(r:get("absu_pnp_mode_1"), 0, "left landing selector off")
    eq(r:get("absu_pnp_mode_2"), 4, "right selector still explicitly LD")
    r:set("absu_speed_mode", 0); r:step()
    eq(r:get("absu_pnp_mode_2"), 0, "right selector off")
end)
test("reset clears LD until landing is reselected", function(r)
    r:capture(); r:press("absu_reset"); r:step(20)
    eq(r:get("absu_pnp_mode_1"), 0, "reset remains effective with preparation on")
    r:set("absu_landing_on", 0); r:step(); r:set("absu_landing_on", 1); r:step()
    eq(r:get("absu_pnp_mode_1"), 4, "landing can be reselected")
end)
...
test("sustained loss cannot recapture from held buttons", function(r)
    ...
    -- Allow the existing five-second signal-loss grace period to expire.
    r:set("nav_cs_flag_1", 1); r:set("nav_gs_flag_1", 1); r:step(260)
...
for _, source in ipairs({1, 2}) do
    test("NAV" .. source .. " automatic GS once per approach", function(r)
        ...
        r:set("nav_gs_flag_" .. source, 1); r:step(260)
