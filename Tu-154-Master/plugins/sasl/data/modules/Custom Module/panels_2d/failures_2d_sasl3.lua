-- Failure DataRefs._2d.lua
-- Tu-154 maintenance and failure service panel.

-- Failure DataRefs. and runtime palette

size = {512, 700}

defineProperty("save_state", globalPropertyi("tu154/custom/save_state")) -- Requests aircraft state saving

-- Panel image resource.
local bg_img = loadImage("repair_tex.png")

defineProperty("show_fail_panel", globalPropertyi("tu154/custom/panels/show_fail_panel")) -- Failure panel visibility


defineProperty("frame_time", globalPropertyf("tu154/custom/time/frame_time")) -- Frame time


local runtime_tbl = {}

-- Remaining component runtimes.
...
customFails["Left outer spoiler"] = globalPropertyi("tu154/custom/failures/fail_spoil_out_left")
customFails["Right outer spoiler"] = globalPropertyi("tu154/custom/failures/fail_spoil_out_right")
...
simFails["!!!Rudder trim"] = globalPropertyi("sim/operation/failures/rel_trim_rud")
simFails["!!!Aileron trim"] = globalPropertyi("sim/operation/failures/rel_trim_ail")
simFails["!!!Elevator trim"] = globalPropertyi("sim/operation/failures/rel_trim_elv")
customFails["!!!Emerg elevator trim"] = globalPropertyi("tu154/custom/failures/trim_emerg_elv_fail")
customFails["!!!Left flaps"] = globalPropertyi("tu154/custom/failures/flap_fail_left")
customFails["!!!Right flaps"] = globalPropertyi("tu154/custom/failures/flap_fail_right")
...
simFails["Front gear retract"] = globalPropertyi("sim/operation/failures/rel_lagear1") -- Landing gear retraction failure
simFails["Left gear retract"] = globalPropertyi("sim/operation/failures/rel_lagear2") -- Landing gear retraction failure
simFails["Right gear retract"] = globalPropertyi("sim/operation/failures/rel_lagear3") -- Landing gear retraction failure
simFails["Gear actuator"] = globalPropertyi("sim/operation/failures/rel_gear_act") -- Landing gear actuator failure
...
customFails["Fuel meter total"] = globalPropertyi("tu154/custom/failures/fuel_meter_summ")
...
customFails["Course MP #1"] = globalPropertyi("tu154/custom/failures/nav1_fail") -- Failure
customFails["Course MP #2"] = globalPropertyi("tu154/custom/failures/nav2_fail") -- Failure
customFails["DME #1"] = globalPropertyi("tu154/custom/failures/dme1_fail") -- Failure
customFails["DME #2"] = globalPropertyi("tu154/custom/failures/dme2_fail") -- Failure
...
customFails["!!!Left pitot block"] = globalPropertyi("tu154/custom/failures/pitot1") -- Left pitot blockage
customFails["!!!Right pitot block"] = globalPropertyi("tu154/custom/failures/pitot2") -- Right pitot blockage
customFails["!!!Left static"] = globalPropertyi("tu154/custom/failures/static1")  -- Static system failure
customFails["!!!Right static"] = globalPropertyi("tu154/custom/failures/static2")  -- Static system failure
simFails["!!!SVS system"] = globalPropertyi("sim/operation/failures/rel_adc_comp")  -- Static system failure

customFails["Control gyro"] = globalPropertyi("tu154/custom/failures/mgv_fail")
...
customFails["Radioaltimeter #1"] = globalPropertyi("tu154/custom/failures/rv1_fail")  -- Failure
customFails["Radioaltimeter #2"] = globalPropertyi("tu154/custom/failures/rv2_fail")  -- Failure
customFails["AOA"] = globalPropertyi("tu154/custom/failures/AOA") -- Failure
simFails["AOA alarm"] = globalPropertyi("sim/operation/failures/rel_stall_warn") -- Failure
customFails["Feet altimeter"] = globalPropertyi("tu154/custom/failures/uvid15_fail") -- Failure
simFails["Left VVI"] = globalPropertyi("sim/operation/failures/rel_ss_vvi") -- Failure
simFails["Right VVI"] = globalPropertyi("sim/operation/failures/rel_cop_vvi") -- Failure
...
-- Refreshes runtime, wear, fluid and active-failure display tables.
local function readAll()
	
	-- Update displayed runtime values.
	for k,v in ipairs(runtime_tbl) do
		runTimeDrawTbl[k] = {v[1], string.format("%.1f", get(v[2]) / 3600)}
	end

	-- Update displayed brake wear values.
	for k,v in ipairs(brakes_tbl) do
		brakesShowTbl[k] = {v[1], string.format("%.3f", get(v[2]))}
	end

	-- Update displayed engine oil quantities.
	for k,v in ipairs(liqd_tbl) do
		liqd_show_tbl[k] = {v[1], string.format("%.1f", get(v[2]))}
	end
		
	-- Rebuild the active failure list.
	failDrawTbl = {} -- Clear the previous failure list.
	...
	for k,v in pairs(simFails) do -- Scan X-Plane failures.
		if get(v) == 6 then
			table.insert(failDrawTbl, k)
		end
	end

	-- Keep the displayed failure list deterministic.
	table.sort(failDrawTbl)
end

-- Clears all configured failures and restores hydraulic fluid quantities.
...
-- Refreshes maintenance display data once per second.
function update()
...
components = {
	...
	-- Engine/APU runtime repair button.
	rectangle {
...
-- Draws all child components of the maintenance panel.
function draw()
	drawAll(components)
end
